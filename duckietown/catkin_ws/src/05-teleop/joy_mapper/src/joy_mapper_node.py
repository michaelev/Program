...
class JoyMapper(object):
	...
	GPIO.setup(self.tombol, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
	...
	self.pub_overtake_on = rospy.Publisher("~overtake_on",BoolStamped,queue_size=1)
	self.pub_overtake_off = rospy.Publisher("~overtake_off",BoolStamped,queue_size=1)
...
# XXX: here we should use constants
	...
	elif (joy_msg.buttons[0] == 1): #tombol A
	    overtake_on = BoolStamped()
	    rospy.loginfo('Overtake ON')
	    overtake_on.data = True
	    self.pub_overtake_on.publish(overtake_on)
	elif (joy_msg.buttons[1] == 1): #tombol B
	    overtake_off = BoolStamped()
	    rospy.loginfo('Overtake OFF')
	    overtake_off.data = True
	    self.pub_overtake_off.publish(overtake_off)
	elif GPIO.input(self.tombol):
	    override_msg = BoolStamped()
	    override_msg.header.stamp = self.joy.header.stamp
	    override_msg.data = False
	    rospy.loginfo('override_msg = False')
